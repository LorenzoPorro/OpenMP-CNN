...
vector<vector<double>> upsample(vector<vector<double>> backPropagatedError, vector<vector<double>> scale)
{
    ...
    if (scale.size() % backPropagatedError.size() != 0 || scale[0].size() % backPropagatedError[0].size() != 0)
    {
        ...
        //cout << "rescale: " << rescale.size() << ", "<<rescale[0].size() << endl;
...
#pragma omp for collapse(4)
            ...
            //cout << "temp: " << temp.size() << ", " << temp[0].size() << endl;
            //cout << "upsample matrix init" << endl;
#pragma for collapse(2)
            ...
            //cout << "upsample: " << upsampleValuesMatrix.size() << ", " << upsampleValuesMatrix[0].size() << endl;
        }
    }
    else
    {
...
#pragma omp for collapse(2)
    ...
    cout << endl
         << "Time elapsed: " << double(clock() - begin) / CLOCKS_PER_SEC << endl;
    cout << endl;
...
int main()
{
    ...
    layerBackValues(forwardValues, backwardValues, layerWeights);
